...
const IxCard: FC<CardProps> = (props) => {
  const { item, className = '' } = props;
  const {
    tokenId,
    auctionData: { id, currBid = '', status, endTime, winner, winningBid, bids },
    auctionData,
    name,
    thumbnail,
  } = item || {};
  ...
  const reservePrice = ethers.utils.parseEther('100').toString();
    ...
    <div className={className}>
      <div className="relative flex flex-col">
        ...
        <div className={`${!condensedLayout && 'h-28 flex flex-col justify-end'}`}>
          <ActiveAuctionCardSection className="flex flex-col" show={auctionIsActive && !condensedLayout} endTime={endTime} currentBid={currBid} auctionId={id} />
          <PendingAuctionCardSection className="flex flex-col" show={auctionIsReady && !condensedLayout} reservePrice={reservePrice} auctionId={id} />
          <SettlingAuctionCardSection className="flex flex-col" show={!!(status === 'settling' && hasAuctionData && !condensedLayout)} bids={filteredBids} />
