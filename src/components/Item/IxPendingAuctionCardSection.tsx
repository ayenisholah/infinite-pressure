...
const IxPendingAuctionCardSection: FC<PendingAuctionCardSectionProps> = (props) => {
  const { reservePrice, className = '', show, auctionId = '' } = props;

  return show ? (
    <div className={className}>
      <span className="mt-1 text-sm">
        Minimum Bid <span className="font-bold">{`${gweiToEth(reservePrice)} ETH`}</span>
      </span>
...
export default IxPendingAuctionCardSection;
